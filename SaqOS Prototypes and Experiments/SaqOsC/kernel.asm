...
welcome:
    mov al, [bx]
    cmp al, 0
    je ask_keyboard ;jmp to handle_newline after writing ask_keyboard stuff
...
ask_keyboard:
    ;if cl is set to 1, it means its in us mode, if its set to 2, its in ch mode
    ...
    ;lowkey i wanna write all the input handler from scratch again, i dont wanna use saqkuserin here, it should be minimal
    
    mov ah, 0x00
    int 0x16 ;now al should have the ascii char 
    mov ah, 0x0E
    int 0x10 ;either 1 or 2 should be on disp
    cmp al, '1'
    je pr_us 
    cmp al, '2'
    je pr_ch_stp 
...
pr_us:
    mov cl, 1 ;flag set to us here
    call saqknewline
    call saqknewline
    mov si, ans_keyb1 ;null terminated str in si
    call saqkprint

    jmp handle_newline

pr_ch_stp:
    mov cl, 2 ;flag set to ch here
    call saqknewline
    call saqknewline
    mov si, ans_keyb2
    call saqkprint
...
pr_ch1: ;the cursor reprinter incase of command finish, jmp here
    call saqknewline
    call saqknewline
    mov si, cmd
    call saqkprint
...
pr_ch3:
    ;we still need register si so we can reuse handle_enter_n_check and other command processing
    cmp al, 'y'
    je swap_y_to_z
...
pr_ch5:
    ...
    inc si
    cmp al, 0x0D
    je handle_enter_n_check ;bruh after one command, keyboard will turn us
    ;yk what, lets use a random unused register to set a flag
    cmp al, 0x08
    je handle_bakspc
    cmp al, 0x20
    je handle_spc
...
handle_newline: ;commands/programms jmp here after they finish, 
    cmp cl, 2
    je pr_ch1
    mov ah, 0x0E
...
handle_bakspc:
 	mov cx, userbuffer ;shi we cant use cx as flag setter, hm what random register can i use?????'?' 
...
cmd_clear:
    ...
    jmp handle_newline ;every command that finishes goes back to handle_newline, if its in ch mode, 
...
ask_keyb: db "Which Layout you want to use? US Qwerty (1), swiss Qwertz (2)> ", 0
ans_keyb2: db "Switched to Swiss Qwertz Layout!", 0
ans_keyb1: db "Switched to US Qwerty Layout!", 0

;output of commands here:
help_out: db "All Commands: src, help, exit, sysinfo, panic, echo, reboot, clear, test. (USE PANIC CAREFULLY!, IT PANICS THE KERNEL!) (more commands to come and direct ring 0 lib calls) also, just type echo and then you will get a prompt on what you want to echo.", 0
src_out: db "Source Code: https://github.com/theresistance-16/SaqOS", 0
saqver_out: db "kernel version: 5 Saqbootloader version: 4", 0
...
userbuffer resb 512 
keyb resb 24 ;why the heck did i define a keyb buffer???

;i used resb in version 4 instead so it doesnt make my kernel over 1 mb already

;section .text
;times (512 * 8) - ($ - $$) db 0 ;pad the kernel out so it corresponds to the sector count (8 sectors long, 8 x 512 bytes)
;temporaly comment those out for testing purposes