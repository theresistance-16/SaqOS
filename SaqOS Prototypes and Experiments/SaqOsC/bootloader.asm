...
boot_to_kernel: 
    ...
    mov al, 10 ;read 10 sectors in total (which all is our kernel)

    mov ch, 0 ;cylinder 0, whatever that is

    mov dh, 0 ;head 0, also dont know what that is

    mov cl, 2 ;start reading the kernel off sector 2

    int 0x13 ;bios now copies the sector that the kernel is in bootable usb to ram at address 0x1000:0x0000
...
welcome db "Welcome to SaqOs Bootloader Version 4!", 0
ask_for_y db "Boot SaqOs? Y/n> ", 0
